...
type LayoutType = 'standard' | 'opposed' | 'side' | 'cross';
...
@Component({
    ...
    <main class="game-container" [ngClass]="['grid-' + players(), 'layout-' + layout()]">
      ...
      >
        <div class="player-box" [style.transform]="getRotation(i)">
          <button class="adjust-btn minus" (click)="adjustLife(i, -1)">-</button>
          <div class="life-display" [style.fontSize]="getFontSize(i)">{{ p.life() }}</div>
          <button class="adjust-btn plus" (click)="adjustLife(i, 1)">+</button>
        </div>
      </section>
        ...
        <div class="menu-card">
          ...
          <ng-container *ngIf="menuStep() === 'layout'">
            ...
            <div class="selector-grid">
              ...
              <button *ngIf="players() >= 2" (click)="nextStep('life', undefined, 'side')">SIDE-BY-SIDE</button>
            </div>
          </ng-container>
  ...
  styles: [`
    ...
    .grid-5 .player-0 { grid-column: span 2; border-bottom: 2px solid #000; }
    .grid-5 .player-1, .grid-5 .player-3 { border-right: 2px solid #000; border-bottom: 2px solid #000; }
    .grid-5 .player-2, .grid-5 .player-4 { border-bottom: 2px solid #000; }

    .grid-6 { grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr 1fr; }
    .grid-6 .player { border-bottom: 2px solid #000; }
    .grid-6 .player:nth-child(odd) { border-right: 2px solid #000; }

    /* Layout: CROSS (Actual Cross Layout - 4 Players) [ | - | ] */
    .layout-cross.grid-4 {
      grid-template-columns: 1fr 2fr 1fr;
      grid-template-rows: 1fr 1.5fr 1fr;
    }
    .layout-cross .player-0 { grid-area: 1 / 2; border-bottom: 2px solid #000; border-left: 2px solid #000; border-right: 2px solid #000; } /* Top */
    .layout-cross .player-1 { grid-area: 1 / 1 / 4 / 2; border-right: 2px solid #000; } /* Left Column */
    .layout-cross .player-2 { grid-area: 1 / 3 / 4 / 4; border-left: 2px solid #000; }  /* Right Column */
    .layout-cross .player-3 { grid-area: 3 / 2; border-top: 2px solid #000; border-left: 2px solid #000; border-right: 2px solid #000; }    /* Bottom */

    /* Layout-specific tweaks */
    .layout-side.grid-2 { grid-template-columns: 1fr 1fr; grid-template-rows: 1fr; }
    .layout-side.grid-2 .player-0 { border-bottom: none; border-right: 2px solid #000; }

    .player { background: #fff; display: flex; align-items: center; justify-content: center; overflow: hidden; }
    .player-box { display: flex; width: 100%; height: 100%; align-items: center; }

    .life-display { flex: 2; font-weight: 900; text-align: center; line-height: 1; pointer-events: none; }
    .adjust-btn {
      flex: 1; height: 100%; font-size: 2rem; border: none; background: transparent;
      color: #000; font-weight: bold; touch-action: manipulation;
    }

    .minus { border-right: 1px solid #ddd; }
    .plus { border-left: 1px solid #ddd; }

    .central-menu-btn {
      position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
      z-index: 50; background: #fff; border: 2px solid #000; width: 80px; height: 50px;
    ...
    .selector-grid button { height: 65px; font-size: 1.2rem; border: 2px solid #000; background: #fff; font-weight: bold; }

    .close-btn { width: 100%; height: 50px; background: #000; color: #fff; font-weight: bold; margin-top: 10px; border: none; }

    .adjust-btn:active, .central-menu-btn:active, .selector-grid button:active {
...
export class App {
  ...
  public nextStep(step: MenuStep, p?: number, l?: LayoutType): void {
    ...
    if (step === 'layout' && (this.tempPlayers === 1)) {
      this.tempLayout = 'standard';
  ...
  public getRotation(index: number): string {
    ...
    if (l === 'cross' && n === 4) {
      ...
      if (index === 1) return 'rotate(90deg)';   // Left
      if (index === 2) return 'rotate(-90deg)';  // Right
      return 'rotate(0deg)';                      // Bottom
    }
    if (n === 1 || l === 'side') return 'rotate(0deg)';
    if (l === 'opposed') {
      if (n === 2 || n === 3) return index === 0 ? 'rotate(180deg)' : 'rotate(0deg)';
      if (n === 4) return index < 2 ? 'rotate(180deg)' : 'rotate(0deg)';
      if (n === 5) return index < 1 ? 'rotate(180deg)' : 'rotate(0deg)';
      if (n === 6) return index < 2 ? 'rotate(180deg)' : 'rotate(0deg)';
    }
    if (l === 'standard') {
      if (n === 2) return index === 0 ? 'rotate(180deg)' : 'rotate(0deg)';
      if (n >= 3) return index < (n > 4 ? 2 : 1) ? 'rotate(180deg)' : 'rotate(0deg)';
    }
    return 'rotate(0deg)';
  }
  ...
  public getFontSize(index: number): string {
    ...
    if (l === 'cross' && n === 4) {
      return (index === 1 || index === 2) ? '15vw' : '15vh';
    }
